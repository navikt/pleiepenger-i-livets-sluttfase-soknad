...
export const oppdaterSøknadMedArbeidsgivere = (
    arbeidsgivere: Arbeidsgiver[],
    { values, setFieldValue }: FormikProps<SoknadFormData>
) => {
    ...
    setFieldValue(SoknadFormField.ansatt_arbeidsforhold, ansattArbeidsforhold);

    const frilansoppdrag = arbeidsgivere.filter(erFrilansoppdrag);
    setFieldValue(SoknadFormField.frilansoppdrag, frilansoppdrag);
};
